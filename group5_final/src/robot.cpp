      ...
      planning_scene_()
{
    ...
    init_conv_parts_sub_ = this->create_subscription<ariac_msgs::msg::ConveyorParts>(
        "/ariac/conveyor_parts", rclcpp::SensorDataQoS(),
        std::bind(&Robot::init_conv_parts_cb, this, std::placeholders::_1), options);

    init_bin_parts_sub_ = this->create_subscription<ariac_msgs::msg::BinParts>(
        "/ariac/bin_parts", rclcpp::SensorDataQoS(),
        std::bind(&Robot::init_bin_parts_cb, this, std::placeholders::_1), options);
    ...
    const ariac_msgs::msg::AdvancedLogicalCameraImage::ConstSharedPtr msg)
{
    ...
    if (conveyor_camera_parts_.size() > 0)
    {
        ...
        // update remaining conveyer part
        if (conveyor_part_detected == false)
        {
            remaining_conveyor_parts[current_conveyor_part.part.color][current_conveyor_part.part.type - 10] -= 1;
            RCLCPP_INFO(get_logger(), "detuct part from conveyor belt");
        }
...
void Robot::init_conv_parts_cb(
    ariac_msgs::msg::ConveyorParts msg)
{
    if (initial_conveyer_parts_received == false)
    {
        int size = msg.parts.size();
        for (int i = 0; i < size; i++)
        {
            remaining_conveyor_parts[msg.parts.at(i).part.color][msg.parts.at(i).part.type - 10] = msg.parts.at(i).quantity;
            RCLCPP_INFO_STREAM(get_logger(), "Conveyor Contains " << msg.parts.at(i).quantity << " x " << part_colors_[msg.parts.at(i).part.color] << " " << part_types_[msg.parts.at(i).part.type]);
        }
        initial_conveyer_parts_received = true;
    }
}

void Robot::init_bin_parts_cb(
    ariac_msgs::msg::BinParts msg)
{
    if (initial_bin_parts_received == false)
    {
        RCLCPP_INFO(get_logger(), "Received data from Initial Bin Topic");
        auto binInfo = msg.bins;

        // read topic, build Occupied Bins List
        for (auto bins : binInfo)
        {
            occupied_bins.push_back(bins.bin_number);
            RCLCPP_INFO(get_logger(), "   - Bin # %d is Occupied.", bins.bin_number);
        }

        // compare All bins and Occupied Bins, the difference is Empty Bins
        for (const auto &elem : all_bins)
        {
            if (std::find(occupied_bins.begin(), occupied_bins.end(), elem) == occupied_bins.end())
            {
                empty_bins.push_back(elem);
            }
        }

        // print Empty Bin numbers
        RCLCPP_INFO(get_logger(), " ------------------------------------------------ ");
        for (const auto &elem : empty_bins)
        {
            RCLCPP_INFO(get_logger(), "   - Bin # %d is Empty.", elem);
            for (int j = 0; j < 9; j++)
            {
                double slot_x = bin_centers[elem].first + slot_offsets[j].first;
                double slot_y = bin_centers[elem].second + slot_offsets[j].second;
                // Slot_XY coord = {slot_x, slot_y};
                empty_bin_poses.push_back({slot_x, slot_y});
            }
        }

        initial_bin_parts_received = true;
    }
}
...
{
    RCLCPP_INFO(get_logger(), " ---------------------------------------------------------------------------- ");
    RCLCPP_INFO_STREAM(get_logger(), "Pick up a Faulty" << part_colors_[part_to_pick.color] << " " << part_types_[part_to_pick.type]);
    RCLCPP_INFO(get_logger(), " ---------------------------------------------------------------------------- ");
    ...
    RCLCPP_INFO(get_logger(), " Quadrant pose x y, --> (%f , %f)", part_pick_pose.position.x, part_pick_pose.position.y);
    RCLCPP_INFO(get_logger(), " ---------------------------------------------------------------------------- ");
    ...
    for (auto part : agv_parts)
    {
        if (part.part.color == part_to_pick.color && part.part.type == part_to_pick.type)
        {
            ...
            RCLCPP_INFO(get_logger(), " Part at --> (%f , %f)", part_pose.position.x, part_pose.position.y);
            RCLCPP_INFO(get_logger(), " Distance to Part --> %f", distance);

            // if (distance < min_distance)
            // {
            //     RCLCPP_INFO(get_logger(), " New Minimum Distance. Part at --> (%f , %f)", part_pose.position.x, part_pose.position.y);
            //     min_distance = distance;
            //     part_pose = trial_pose
            // }
    ...
    double robot_rotation = GetYaw(part_pose);

    RCLCPP_INFO(get_logger(), "Move closer to AGV Part");
    std::vector<geometry_msgs::msg::Pose> waypoints;

    waypoints.push_back(BuildPose(part_pose.position.x - 0.03,
    ...
    if (FloorRobotWaitForAttach(5.0) == false)
    {
    ...
    FloorRobotMoveCartesian(waypoints, 0.3, 0.3);
    // FloorRobotMoveCartesian(waypoints, 0.7, 0.7);
...
bool Robot::FloorRobotDisposePart(int agv_num)
{
    ...
    // else if (agv_num == 1)
    // {
    //     RCLCPP_INFO(get_logger(), "Move to Right Disposal Bin Position");
    //     floor_robot_.setJointValueTarget(right_disposal_bin_js);
    //     FloorRobotMovetoTarget();
    //     floor_robot_.setJointValueTarget("linear_actuator_joint", rail_positions_["agv" + std::to_string(agv_num)]);
    //     floor_robot_.setJointValueTarget("floor_shoulder_pan_joint", 0);
    // }
    ...
    // else if (agv_num == 4)
    // {
    //     RCLCPP_INFO(get_logger(), "Move to Left Disposal Bin Position");
    //     floor_robot_.setJointValueTarget(left_disposal_bin_js);
    //     FloorRobotMovetoTarget();
    //     floor_robot_.setJointValueTarget("linear_actuator_joint", rail_positions_["agv" + std::to_string(agv_num)]);
    //     floor_robot_.setJointValueTarget("floor_shoulder_pan_joint", 0);
    // }
    ...
    // if (FloorRobotMovetoTarget() == false)
    // {
    //     RCLCPP_INFO(get_logger(), "Failed to move to target, sending home");
    //     FloorRobotSendHome();
    // }
...
bool Robot::KittingPartQualityCheck(ariac_msgs::msg::KittingTask task)
{
    ...
    {
        RCLCPP_WARN_STREAM(get_logger(), "quality_check failed");
        ...
        // agv_camera_poses_ = agv_num_camera_poses_[agv_num];
        // agv_camera_parts_ = agv_num_camera_parts_[agv_num];
...
bool Robot::FloorRobotMovetoTarget()
{
    ...
    {
        RCLCPP_ERROR(get_logger(), "FloorRobotMovetoTarget() - Unable to generate plan. ");
        return false;
    }
}
    ...
    std::vector<geometry_msgs::msg::Pose> waypoints, double vsf, double asf)
{
    ...
    {
        RCLCPP_ERROR(get_logger(), "FloorRobotMoveCartesian() - Unable to generate trajectory through waypoints");
        return false;
    }
...
bool Robot::FloorRobotWaitForAttach(double timeout)
{
    ...
    while (!floor_gripper_state_.attached)
    {
        ...
        {
            RCLCPP_ERROR(get_logger(), "FloorRobotWaitForAttach() - Unable to pick up object");
            return false;
        }
    }
    RCLCPP_WARN_STREAM(get_logger(), "Gripper Status: " << floor_gripper_state_.attached);
...
void Robot::FloorRobotWaitForAttachConveyor(double timeout)
{
    ...
    while (!floor_gripper_state_.attached)
    {
        ...
        if (now() - start > rclcpp::Duration::from_seconds(timeout))
        {
            RCLCPP_ERROR(get_logger(), "FloorRobotWaitForAttachConveyor() - Unable to pick up object");
            return;
        }
    }
...
bool Robot::FloorRobotSetGripperState(bool enable)
{
    ...
    {
        RCLCPP_ERROR(get_logger(), "FloorRobotSetGripperState() - Error calling gripper enable service");
        return false;
    }
...
bool Robot::FloorRobotChangeGripper(std::string station, std::string gripper_type)
{
    ...
    if (!result.get()->success)
    {
        RCLCPP_ERROR(get_logger(), "FloorRobotChangeGripper() - Error calling gripper change service");
        return false;
    }
...
bool Robot::FloorRobotPickandPlaceTray(int tray_id, int agv_num)
{
    ...
    RCLCPP_INFO(get_logger(), " - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - ");
    RCLCPP_INFO(get_logger(), " Attempting to pick Tray of ID: %i", tray_id);
    RCLCPP_INFO(get_logger(), " - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - ");
    ...
    if (!found_tray)
    {
        RCLCPP_ERROR(get_logger(), "Tray not Found");
        return false;
    }
    ...
    if (!FloorRobotMoveCartesian(waypoints, 0.3, 0.3))
    {
        RCLCPP_ERROR(get_logger(), "Failed to Pick Up Tray");
        return false;
    }
    ...
    if (!FloorRobotWaitForAttach(2.0))
    {
        RCLCPP_ERROR(get_logger(), "Failed to Pick Up Tray");
        return false;
    }
...
bool Robot::FloorRobotPickConveyorPart(ariac_msgs::msg::Part part_to_pick)
{
    ...
    // adjust robot configuration to SLIDE
    ...
    waypoints.push_back(BuildPose(part_pose.position.x,
                                  -pick_up_location - conveyer_pick_offset[part_to_pick.type],
                                  part_pose.position.z + part_heights_[part_to_pick.type] + 0.01,
    ...
    waypoints.push_back(BuildPose(part_pose.position.x,
                                  -pick_up_location - conveyer_pick_offset[part_to_pick.type],
                                  part_pose.position.z + part_heights_[part_to_pick.type] + 0.003,
                                  SetRobotOrientation(part_rotation)));
    waypoints.push_back(BuildPose(part_pose.position.x,
                                  -pick_up_location - conveyer_pick_offset[part_to_pick.type] - 0.3,
                                  part_pose.position.z + part_heights_[part_to_pick.type] - 0.00005,
                                  SetRobotOrientation(part_rotation)));
    waypoints.push_back(BuildPose(part_pose.position.x,
                                  -pick_up_location - conveyer_pick_offset[part_to_pick.type] - 0.5,
                                  part_pose.position.z + part_heights_[part_to_pick.type] + 0.01,
                                  SetRobotOrientation(part_rotation)));
    ...
    while (true)
    {
        ...
        {
            FloorRobotSetGripperState(true);
            FloorRobotMoveCartesian(waypoints, 0.038, 0.1); // velocity factor was 0.023
            waypoints.clear();
            break;
        }
    }
    ...
    current_pose.position.z = part_pose.position.z + part_heights_[part_to_pick.type] + 0.6;
    waypoints.push_back(current_pose);
    ...
    {
        RCLCPP_ERROR(get_logger(), " FloorRobotPickConveyorPart() - Failed to Move away from Pick Location ");
        FloorRobotSetGripperState(false);
        floor_robot_.detachObject(part_name);
        planning_scene_.removeCollisionObjects(std::vector<std::string>{part_name});
        return false;
    ...
    if (!floor_gripper_state_.attached)
    {
        RCLCPP_ERROR(get_logger(), " FloorRobotPickConveyorPart() - No Part attached");        
        FloorRobotSetGripperState(false);
        floor_robot_.detachObject(part_name);
        planning_scene_.removeCollisionObjects(std::vector<std::string>{part_name});
        return false;
    ...
    auto place_xy = empty_bin_poses[conveyor_pick_up_counter];

    waypoints.push_back(BuildPose(place_xy.x - conveyer_pick_offset[part_to_pick.type], place_xy.y,
                                  bin_pose.position.z + 0.3, SetRobotOrientation(0)));
    waypoints.push_back(BuildPose(place_xy.x - conveyer_pick_offset[part_to_pick.type], place_xy.y,
    ...
    // Move up slightly
    waypoints.clear();
    waypoints.push_back(BuildPose(place_xy.x - conveyer_pick_offset[part_to_pick.type],
                                  place_xy.y,
                                  bin_pose.position.z + 0.5,
                                  SetRobotOrientation(0)));
    FloorRobotMoveCartesian(waypoints, 0.3, 0.3);

    conveyor_pick_up_counter++;
...
bool Robot::FloorRobotPickBinPart(ariac_msgs::msg::Part part_to_pick)
{
    ...
    std::vector<geometry_msgs::msg::Pose> waypoints;
    waypoints.push_back(BuildPose(part_pose.position.x - 0.03, part_pose.position.y,
...
// ---------------------------------------------------------------------------- //
// | --------------------- | Initial Conveyor Pick Up | --------------------- | //
// ---------------------------------------------------------------------------- //

void Robot::PickAllConveyorParts()
{

    // wait till conveyor parts information is received
    while (initial_conveyer_parts_received == false)
    {
    }

    int conveyor_parts_total = 0;  // total number of conveyor parts
    int conveyor_part_counter = 0; // counter of conveyor parts

    // get number of total conveyor parts
    RCLCPP_INFO(get_logger(), " Pick up Conveyor Parts:");
    for (int i = 0; i < 5; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            if (remaining_conveyor_parts[i][j] > 0)
            {
                RCLCPP_INFO(get_logger(), "   - %d x %s %s ", remaining_conveyor_parts[i][j], part_colors_[i].c_str(), part_types_[j + 10].c_str());
                conveyor_parts_total += remaining_conveyor_parts[i][j];
            }
        }
    }

    size_t total_empty_spots = empty_bins.size() * 9;
    int total_empty_spots_int = static_cast<int>(total_empty_spots);

    RCLCPP_INFO(get_logger(), "- - - - - - - - - - - - - - - - - - - - - - -");
    RCLCPP_INFO(get_logger(), " Total Conveyor Parts %d", conveyor_parts_total);
    RCLCPP_INFO(get_logger(), " Total Empty Spots %d", total_empty_spots_int);
    RCLCPP_INFO(get_logger(), "- - - - - - - - - - - - - - - - - - - - - - -");

    FloorRobotSendHome();

    while ((conveyor_part_counter != conveyor_parts_total) & (conveyor_part_counter != total_empty_spots_int))
    {
        if (conveyor_camera_parts_.size() > 0)
        {
            auto pick_this_part = conveyor_camera_parts_.at(0);
            ...
            // perform pick up
            FloorRobotPickConveyorPart(pick_this_part.part);

            conveyor_part_counter++;
            RCLCPP_INFO(get_logger(), "     Increment Conveyor Part Counter %d", conveyor_part_counter);
        }
    }
}
...
bool Robot::CompleteOrders()
{
    ...
    PickAllConveyorParts(); // run until conditions are met

    RCLCPP_INFO(get_logger(), "- - - - - - - - - - - - - - - - - - - - - - - - ");
    RCLCPP_INFO(get_logger(), "-         Done Picking Up Conveyor Parts      - ");
    RCLCPP_INFO(get_logger(), "- - - - - - - - - - - - - - - - - - - - - - - - ");
    ...
    while (true)
    {
        ...
        {
            RCLCPP_INFO(get_logger(), "- - - - - - - - - - - - - - - - - - - - - - -");
            RCLCPP_INFO(get_logger(), "             Shutdown Robot Node");
            RCLCPP_INFO(get_logger(), "- - - - - - - - - - - - - - - - - - - - - - -");
        ...
        if (!priority_orders_.empty())
        {
            ...
            {
                RCLCPP_INFO(get_logger(), " ------------------------------------------- ");
                RCLCPP_INFO(get_logger(), " --          Priority - Kitting Task      --");
                RCLCPP_INFO(get_logger(), " ------------------------------------------- ");
                check_insufficient_part(current_order_);
            ...
            {
                RCLCPP_INFO(get_logger(), " -------------------------------------------- ");
                RCLCPP_INFO(get_logger(), " --          Priority - Combined Task      -- ");
                RCLCPP_INFO(get_logger(), " -------------------------------------------- ");
                if (!conveyor_parts_picked)
                {
                    conveyor_parts_picked = true;
        ...
        else if (!orders_.empty())
        {
            ...
            {
                RCLCPP_INFO(get_logger(), " ------------------------------------------- ");
                RCLCPP_INFO(get_logger(), " --          Regular - Kitting Task       -- ");
                RCLCPP_INFO(get_logger(), " ------------------------------------------- ");
                check_insufficient_part(current_order_);
                if (!conveyor_parts_picked)
                {
            ...
            {
                RCLCPP_INFO(get_logger(), " -------------------------------------------- ");
                RCLCPP_INFO(get_logger(), " --           Regular - Combined Task      -- ");
                RCLCPP_INFO(get_logger(), " -------------------------------------------- ");
                if (!conveyor_parts_picked)
                {
                    conveyor_parts_picked = true;
...
bool Robot::CompleteKittingTask(ariac_msgs::msg::KittingTask task)
{
    ...
    {
        agv_track[task.agv_number] = true;
    ...
    else
    {
        ...
        if (priority_orders_.size() != 0)
        {
            ...
            RCLCPP_INFO(get_logger(), " - Priority order recieved before tray ");
            orders_.insert(orders_.begin(), current_order_);
            return false;
        }

        if (resumeFromMethod == "FloorRobotPickandPlaceTray" || current_order_.step.empty())
        {
        ...
        if (resumeFromMethod == "FloorRobotPickandPlacePart")
        {
            ...
            for (auto kit_part : task.parts)
            {
                ...
                if (partTrack[kit_part.quadrant] == current_order_.id)
                {
                    continue;
                }
                FloorRobotPickBinPart(kit_part.part);
                FloorRobotPlacePartOnKitTray(task.agv_number, kit_part.quadrant);
                partTrack[kit_part.quadrant] = current_order_.id;
                if (priority_orders_.size() != 0)
                {
                ...
                {
                    resumeFromMethod = "PerformQualityCheck";
                }
            }
        }
        if (resumeFromMethod == "PerformQualityCheck")
        {
            bool Quality = KittingPartQualityCheck(task);
            if (priority_orders_.size() != 0)
            {
                if (Quality)
                {
...
bool Robot::CompleteCombinedTask(ariac_msgs::msg::CombinedTask task)
{
    ...
    else
    {
        ...
        if (current_order_.step.empty() || resumeFromMethod == "FloorRobotPickandPlaceTray")
        {
            FloorRobotPickandPlaceTray(0, agv_number);
            if (priority_orders_.size() != 0)
            {
                current_order_.step = "PickandPlaceparts";
                orders_.insert(orders_.begin(), current_order_);
                return false;
            }
            else
            {
                resumeFromMethod = "PickandPlaceparts";
            }
        }

        if (resumeFromMethod == "PickandPlaceparts")
        {
            for (auto assembly_part : task.parts)
            {
        ...
        if (resumeFromMethod == "MoveAgv")
        {
            ...
            {
                resumeFromMethod = "CeilingRobotAssemblePart";
            }
        }
        ...
        RCLCPP_INFO(get_logger(), "id %s", current_order_.id.c_str());
        ...
        if (resumeFromMethod == "CeilingRobotAssemblePart")
        {
            ...
            for (auto const &part_to_assemble : task.parts)
            {
                ...
                for (auto const &agv_part : agv_part_poses)
                {
                    if (agv_part.part.type == part_to_assemble.part.type && agv_part.part.color == part_to_assemble.part.color)
                    {
                        auto x = agv_part.pose.position.x;
                        auto y = agv_part.pose.position.y;
                        auto z = agv_part.pose.position.z;
                ...
                // Assemble Part to insert
                CeilingRobotAssemblePart(task.station, part_to_assemble);
                // agv_part_poses.erase(agv_part_poses.begin());
...
// ---------------------------------------------------------------------------- //
// | ------------------ | Insufficient Parts Functions | -------------------- | //
// ---------------------------------------------------------------------------- //

bool Robot::check_insufficient_part(group5_msgs::msg::FloorRobotTask msg)
{
    bool report_insufficient_part = false;
    // store bin parts into 2D array
    int total_parts_array[5][4] = {};
    for (auto part : left_bins_parts_)
    {
        total_parts_array[part.part.color][part.part.type - 10] += 1;
    }
    for (auto part : right_bins_parts_)
    {
        total_parts_array[part.part.color][part.part.type - 10] += 1;
    ...
    RCLCPP_INFO(get_logger(), " - - - - - - - - - - - - - - - - - - - - - - - - - -");
    RCLCPP_INFO(get_logger(), " -           Insufficient Part Challange           -");
    RCLCPP_INFO(get_logger(), " - - - - - - - - - - - - - - - - - - - - - - - - - -");

    // print bin parts
    RCLCPP_INFO(get_logger(), "Bin Parts:");
    for (int i = 0; i < 5; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            if (total_parts_array[i][j] > 0)
            {
                RCLCPP_INFO(get_logger(), "   - %d x %s %s ", total_parts_array[i][j], part_colors_[i].c_str(), part_types_[j + 10].c_str());
            }
        }
    }

    // print conveyor parts and combine to total parts
    RCLCPP_INFO(get_logger(), "Conveyor Parts:");
    for (int i = 0; i < 5; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            total_parts_array[i][j] += remaining_conveyor_parts[i][j];
            if (remaining_conveyor_parts[i][j] > 0)
            {
                RCLCPP_INFO(get_logger(), "   - %d x %s %s ", remaining_conveyor_parts[i][j], part_colors_[i].c_str(), part_types_[j + 10].c_str());
            }
    ...
    // print total parts
    RCLCPP_INFO(get_logger(), "Total Parts:");
    for (int i = 0; i < 5; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
    ...
    // // store previos orders into 2D array
    // int previous_order_parts_array[5][4] = {};
    // for (auto order : orders_)
    // {
    //     if (order.id != msg.id)
    //     {
    //         for (auto part : order.kitting_task.parts)
    //         {
    //             previous_order_parts_array[part.part.color][part.part.type - 10] += 1;
    //         }
    //     }
    // }
    // for (auto order : priority_orders_)
    // {
    //     if (order.id != msg.id)
    //     {
    //         for (auto part : order.kitting_task.parts)
    ...
    //     }
    // }
    // // print previous order parts
    // for (int i = 0; i < 5; ++i)
    // {
    //     for(int j = 0; j < 4; ++j)
    //     {
    //         if (previous_order_parts_array[i][j] > 0)
    //         {
    //             RCLCPP_INFO(get_logger(), "previous order parts: %s %s %d", part_colors_[i].c_str(), part_types_[j + 10].c_str(), previous_order_parts_array[i][j]);
    //         }
    //     }
    // }

    // store incoming order into 2D array
    int order_parts_array[5][4] = {};
    for (auto part : msg.kitting_task.parts)
    {
        order_parts_array[part.part.color][part.part.type - 10] += 1;
    }

    // print order parts
    RCLCPP_INFO(get_logger(), "Required Order Parts:");

    for (int i = 0; i < 5; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            if (order_parts_array[i][j] > 0)
            {
                RCLCPP_INFO(get_logger(), "   - %d x %s %s ", order_parts_array[i][j], part_colors_[i].c_str(), part_types_[j + 10].c_str());
            }
        }
    }

    // insufficient part check
    ...
    for (int i = 0; i < 5; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            remaining_parts_array[i][j] = total_parts_array[i][j] - order_parts_array[i][j];
            if (remaining_parts_array[i][j] < 0)
            {
                if (report_insufficient_part == false)
                {
                    RCLCPP_WARN(get_logger(), "Challenge FAILED: Insufficient Parts to complete Kitting Order!");
                    report_insufficient_part = true;
                }
                RCLCPP_WARN(get_logger(), "Missing Parts:");
                RCLCPP_WARN(get_logger(), "   - %s %s", part_colors_[i].c_str(), part_types_[j + 10].c_str());
            }
        }
    }
    if (report_insufficient_part == false)
    {
        RCLCPP_INFO(get_logger(), "Challenge PASSED: Sufficient Parts to complete Kitting Order!");
    }
    RCLCPP_INFO(get_logger(), " ------------------------------------------- ");
    return report_insufficient_part; // true if insufficient
}
